    ...
    <>
      {blogData.map((blog) => {
        return (
          <HashLink
            key={blog.id}
            smooth to={`/blogs#${blog.id}`}>
            <div
              style={{ backgroundImage: `url(${blog.image})` }}
              className='bg-center bg-cover overflow-hidden
        w-full xl:max-w-6xl h-dvh pt-40 xl:h-auto xl:pt-50 rounded-lg flex items-end
        shadow-lg hover:shadow-xl transition-all duration-400 hover:scale-108 cursor-pointer'>

              <div className="bg-black/70 xl:bg-black/90 p-4 xl:p-10 xl:pl-5">
